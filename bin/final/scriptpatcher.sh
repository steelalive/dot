...
do
	...
	if [ ! -z "$func" ]; then
		...
		if [[ ! $line =~ "apply_raw_image" ]] && [[ ! $line =~ "format" ]] && [[ ! $line =~ "show_progress" ]] && [[ ! $line =~ "apply_patch_check" ]] && [[ ! $line =~ "range_sha1" ]] && [[ ! $line =~ "block_image_recover" ]] && [[ ! $line =~ "print_superblock_info" ]] && [[ ! $line =~ "getprop" ]] && [[ ! $line =~ "apply_patch_space" ]]; then
			#Supported commands
			if [[ $line =~ "mount" ]] || [[ $line =~ "ui_print" ]] || [[ $line =~ "block_image_update" ]] ||
				[[ $line =~ "apply_patch" ]] || [[ $line =~ "delete" ]] || [[ $line =~ "abort" ]] || [[ $line =~ "package_extract_dir" ]]; then
				#remove EMMC: from apply_patch for image files
				if [[ $line =~ "apply_patch" ]] && [[ $line =~ "EMMC:" ]]; then
				...
				#fix mount command
				if [[ $bashC =~ "mount" ]] && [[ ! $bashC =~ "umount" ]]; then
