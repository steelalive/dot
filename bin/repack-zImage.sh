...
gunzipWithTrailer() {
	...
	while :; do
		...
		cmd gunzip -c "$tmpfile" >/dev/null 2>&1
		res=$?
		if [ "$d" -eq 1 ]; then
			: $((at_min++))
	...
	if [ "$at_min" -gt 3 ]; then
		...
		for ((d = 1; d < 30; d++)); do
			...
			echo -n "  $size"
			rqd dd if="$file" of="$tmpfile" bs=$size count=1 2>/dev/null
			if cmd gunzip -c "$tmpfile" >/dev/null 2>&1; then
				echo -n " - OK"
				fini=1
				break
			fi
	...
	rqd dd if="$file" of="$gz_result" bs=$real_end count=1 2>&9
	rqd dd if="$file" of="$padding" skip=$real_end bs=1 count=$((real_next_start - real_end)) 2>&9
	rqd dd if="$file" of="$trailer" bs=$real_next_start skip=1 2>&9
...
unlzmaWithTrailer() {
	...
	while :; do
		rqd dd if="$file" of="$tmpfile" bs=$size count=1 2>/dev/null
		cmd unlzma -S '' -c "$tmpfile" >/dev/null 2>&1
		res=$?
...
padTo() {
	...
	if [ "$size" -eq "$2" ]; then
		...
		rqd dd if=/dev/zero of="$tmp" count=1 bs=$(($2 - size)) 2>&9
		rqd cat "$tmp" >>"$1"
		rqd rm -f "$tmp"
	fi
}
...
your own there, the  repacking result may not reflect the result of the
	...
	rqd dd if="$cur_dir/$zImage" bs="$piggy_start" count=1 of="$decompression_code" 2>&9
	rqd dd if="$cur_dir/$zImage" bs="$piggy_start" skip=1 of="$piggy_gz_piggy_trailer" 2>&9
	...
	if [ "$compressed" ]; then
		rqd dd if="$piggy" of="$kernel_img" bs="$ramfs_gz_start" count=1 2>&9
		rqd dd if="$piggy" of="$ramfs_gz_part3" bs="$ramfs_gz_start" skip=1 2>&9
		if [ "$compressed" = "gz" ]; then
		...
		rqd dd if="$piggy" of="$kernel_img" bs="$initrd_start" count=1 2>&9
		rqd dd if="$piggy" of="$ramfs_part3" bs="$initrd_start" skip=1 2>&9
		...
		rqd dd if="$ramfs_part3" of="$ramfs_cpio" bs="$initrd_end" count=1 2>&9
		...
		echo $padding3_len
		rqd dd if="$ramfs_part3" of="$padding3" skip="$initrd_end" count="$padding3_len" bs=1 2>&9
		rqd dd if="$ramfs_part3" of="$part3" bs="$real_next_start" skip=1 2>&9
